...
class NerChunkModel(Component):
    ...
    def __call__(self, text_batch_list: List[List[str]],
        ...
        for text_batch, sentences_offsets_batch, sentences_batch in \
            ...
            log.info(f"ner time {tm_ner_end - tm_ner_st}")
            log.info(f"entity_substr_batch {entity_substr_batch}")
            log.info(f"entity_positions_batch {entity_positions_batch}")
            ...
            log.info(f"entity_substr_batch {entity_substr_batch}")
            log.info(f"entity_offsets_batch {entity_offsets_batch}")
...
class EntityLinkerSep(Component, Serializable):
    ...
    def __init__(self, load_path: str,
                 ...
                 type_to_tag_filename: str,
                 type_to_label_filename: str,
                 ...
                 log_filename: str = "/data/log_faiss.txt",
                 **kwargs) -> None:
        """
        ...
        self.type_to_tag_filename = type_to_tag_filename
        self.type_to_label_filename = type_to_label_filename
        ...
        if self.fit_tfidf_vectorizer:
            self.log_to_file("started tfidf vectorizer")
            ...
            self.log_to_file("tfidf vectorizer, transformed")
        ...
        if self.fit_fasttext_vectorizer:
            ...
            with tqdm(total=len(self.labels_list)) as pbar:
                for label in self.labels_list:
                    ...
                    pbar.update(1)
            self.log_to_file("fasttext vectorizer, processed")
    ...
    def load(self) -> None:
        ...
        self.type_to_tag = load_pickle(self.load_path / self.type_to_tag_filename)
        self.type_to_label = load_pickle(self.load_path / self.type_to_label_filename)
    ...
    def __call__(self, entity_substr_batch: List[List[str]],
        ...
        entity_wiki_types_batch, entity_wiki_tags_batch = self.find_types(entity_ids_batch, tags_batch)
        ...
        if self.return_confidences:
            return entity_substr_batch, conf_batch, entity_offsets_batch, entity_ids_batch, tags_batch, \
                   entity_labels_batch, entity_wiki_types_batch, entity_wiki_tags_batch, status_batch
        else:
            return entity_substr_batch, entity_offsets_batch, entity_ids_batch, tags_batch, entity_labels_batch, \
                entity_wiki_types_batch, entity_wiki_tags_batch, status_batch
    ...
    def link_entities(self, entity_substr_batch: List[List[str]],
        ...
        log.info(f"entity substr batch {entity_substr_batch}")
        log.info(f"entity offsets batch {entity_offsets_batch}")
            ...
            if entity_substr_list:
                try:
                    ...
                    for entity_substr, candidate_entities, candidate_entities_ft \
                        ...
                        log.info(f"{entity_substr} candidate_entities before ranking {candidate_entities[:10]}")
                        ...
                        log.info(f"candidate_entities {candidate_entities[:10]}")
                        ...
                        conf = [candidate_entity[1:] for candidate_entity
                                in candidate_entities][:self.num_entities_for_bert_ranking]
                        log.info(f"{entity_substr} candidate_entities before bert ranking {candidate_entities[:10]}")
                    ...
                    log.info(f"search by index time {tm_ind_end - tm_ind_st}")
                    ...
                    log.info(f"description time {tm_descr_end - tm_descr_st}")
                except:
                    if self.num_entities_to_return == 1:
    ...
    def rank_by_description(self, entity_substr_list: List[str],
        ...
        log.info(f"rank, entity pos {entity_offsets_list}")
        ...
        for sample_num, (entity_substr, (entity_start_offset, entity_end_offset), candidate_entities) in \
            ...
            log.info(f"entity_offsets {entity_start_offset}, {entity_end_offset}")
            log.info(f"candidate_entities {candidate_entities[:10]}")
            sentence = ""
            rel_start_offset = 0
            rel_end_offset = 0
            ...
            for num, (sent, (sent_start_offset, sent_end_offset)) in \
                ...
                if entity_start_offset >= sent_start_offset and entity_end_offset <= sent_end_offset:
                    ...
                    break
            log.info(f"rank, found sentence {sentence}")
            log.info(f"rank, relative offsets {rel_start_offset}, {rel_end_offset}")
            context = ""
            if sentence:
    ...
    def find_types(self, entity_ids_batch: List[List[List[str]]], tags_batch: List[List[str]]):
        entity_wiki_types_batch, entity_wiki_tags_batch = [], []
        for entity_ids_list, entity_tags_list in zip(entity_ids_batch, tags_batch):
            entity_wiki_types_list, entity_wiki_tags_list = [], []
            for entity_ids, entity_tag in zip(entity_ids_list, entity_tags_list):
                if isinstance(entity_ids, list):
                    ...
                    for elem in entity_types_tags:
                        if elem:
                            ...
                            entity_types_tags_unique.append(entity_tag)
                    ...
                    if entity_types_tags:
                        ...
                        entity_types_tags_unique = entity_tag
                    entity_types_labels = [self.type_to_label.get(tp, "") for tp in entity_types]
                else:
                    entity_types_tags = [entity_tag for _ in entity_ids]
