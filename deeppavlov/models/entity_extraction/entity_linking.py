...
class EntityLinker(Component, Serializable):
    ...
    def __call__(self, entity_substr_batch: List[List[str]],
        ...
        for entity_substr_list, entity_offsets_list, entity_tags_list, sentences_list, sentences_offsets_list in \
            ...
            entity_ids_list = self.link_entities(entity_substr_list, entity_offsets_list, entity_tags_list, sentences_list,
                                                 sentences_offsets_list)
            entity_ids_batch.append(entity_ids_list)
        return entity_substr_batch, entity_ids_batch
    ...
    def process_cand_ent(self, cand_ent_init, entities_and_ids, entity_substr_split):
        if self.use_tags:
    ...
    def find_exact_match(self, entity_substr, tag):
        ...
        if self.lang == "@ru":
            ...
            if entity_substr_lemm != entity_substr:
                ...
                entities_and_ids = res.fetchall()
                if entities_and_ids:
                    cand_ent_init = self.process_cand_ent(cand_ent_init, entities_and_ids, entity_substr_split_lemm)
        return cand_ent_init
        ...
        for word in entity_substr_split:
            ...
            if self.lang == "@ru":
                ...
                if word != word_lemm:
                    ...
                    part_entities_and_ids = res.fetchall()
                    cand_ent_init = self.process_cand_ent(cand_ent_init, part_entities_and_ids, entity_substr_split_lemm)
        return cand_ent_init
