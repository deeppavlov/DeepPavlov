...
class BertNerPreprocessor(Component):
    """
    Takes tokens and splits them into bert subtokens, encodes subtokens with
    their indices.
    Creates mask of subtokens (equals one for first subtoken of a word, zero
    for the following subtokens).
    ...
    def _ner_bert_tokenize(tokens: List[str],
        ...
        for token, tag in zip(tokens, tags):
            ...
            if not subwords or (do_cutting and (len(subwords) > max_subword_len)):
                ...
                if do_masking and (random.random() < token_maksing_prob):
