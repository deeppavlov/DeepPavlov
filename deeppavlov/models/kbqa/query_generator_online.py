...
class QueryGeneratorOnline(QueryGeneratorBase):
    ...
    def query_parser(self, question: str, query_info: Dict[str, str],
        ...
        entities_and_types_select = query_info["entities_and_types_select"].get(f"{len(entity_ids)} {len(type_ids)}", [])
        ...
        for selection_var in entities_and_types_select:
            entity_positions, type_positions = [elem.split('_') for elem in selection_var.split(' ')]
