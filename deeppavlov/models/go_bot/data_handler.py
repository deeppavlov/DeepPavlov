...
class TokensVectorizer:
    ...
    def _use_bow_encoder(self) -> bool:
        """
        :returns: is BOW encoding enabled in the TokensVectorizer
        """
    ...
    def _embed_tokens(self, tokens: List[str], mean_embeddings: bool) -> Optional[np.ndarray]:
        ...
        :param mean_embeddings: if True, will return the mean vector of calculated embeddings sequence.
        ...
        tokens_embedded = None  # todo decide, [], None or np.array([]) we use in such cases
    ...
    def bow_encode_tokens(self, tokens: List[str]) -> np.ndarray:
        """
        :param tokens: list of tokens to BOW encode

        :returns: if uses BOW encoder, returns np array with BOW encoding for tokens.
                  Otherwise returns an empty list.
        """
        bow_features = []  # todo decide, [], None or np.array([]) we use in such cases
    ...
    def _standard_normal_like(source_vector: np.ndarray) -> np.ndarray:
        """
        :param source_vector: the vector of which to follow the result shape
        :returns: the standard normal distribution of the shape of the source vector
        """
    ...
    def _pad_sequence_to_size(out_sequence_length: int, token_dim: int, tokens_embedded: np.ndarray) -> np.ndarray:
        ...
        :param out_sequence_length: the length to pad sequence to
        :param token_dim: the shape of output embedding
        :param tokens_embedded: some sequence of vectors
        :returns: the padded sequence of vectors
        """
    ...
    def calc_tokens_mean_embedding(self, tokens: List[str]) -> np.ndarray:
        """
        :param tokens: list of tokens to embed
        :returns: the average vector of embeddings sequence
    ...
    def calc_tokens_embeddings(self, output_sequence_length: int, token_dim: int, tokens: List[str]) -> np.ndarray:
        """
        Calculate embeddings of passed tokens. Returns the padded sequence of calculated embeddings.
        :param output_sequence_length: the length of sequence to output
        :param token_dim: the shape of output embedding
        :param tokens: list of tokens to embed
        :returns: the padded sequence of calculated embeddings
        """
        ...
        if tokens_embedded is not None:
            ...
            emb_context = np.array([], dtype=np.float32)  # todo decide, [], None or np.array([]) we use in such cases
        return emb_context

    def get_dims(self) -> TokensVectorRepresentationParams:
        """
        :returns: the TokensVectorRepresentationParams with embedder and BOW encoder output dimensions.
