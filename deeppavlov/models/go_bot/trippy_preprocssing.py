...
def normalize_text(text):
    ...
    )  # These are very necessary, because there are discrepencies btw the TripPy slot values & the actual text
    ...
    text = re.sub(
        "asian ori$", "asian oriental", text
    )  # Cannot remove the space cuz else it will mess with normal asian ori; probably need some more regex
...
def get_sys_inform(response, slot_type):
    ...
    return (
        "inform_" + slot_type in response["act"]
        if (response is not None) and (response["act"] is not None)
        else False
    )
        ...
        if value == "dontcare" and (
            ...
            if not in_usr and not in_sys:
                ...
                if slot_type not in prev_ds_dict or value != prev_ds_dict[slot_type]:
                    ...
                    print(
                        "Value: {} for slot type {} does not exist in utterance: {}".format(
                            value, slot_type, usr_utt_tok
                        )
                    )
                    print(
                        "Other Information: ", prev_ds_dict, usr_slot_label, sys_utt_tok
                    )
                    print(
                        "Most likely have to add the incorrect utterance token to be replaced with the value see normalizations in the code."
                    )
                    print(
                        "This error is likely because of a mismatch in the slot value & the actual text!\n \
        ...
        for intent in context["intents"]:
            if (intent["slots"] is not None) and (len(intent["slots"]) > 0):
                if intent["act"] == "request":
                    ...
                    slot_dict = {"exclusive_end": -1, "slot": slot, "start": -1}
                    ...
                    if set(value_tok) <= set(usr_utt_tok):
                        ...
                        searcher = LevenshteinSearcherComponent(
                            usr_utt_tok, max_distance=10
                        )
                        ...
                        if top_candidate not in usr_utt_tok:
                            top_candidate = usr_utt_tok[
                                0
                            ]  # Just randomly take the first token
...
def get_turn_label(
    ...
    (
        ...
        usr_utt_tok,
        usr_slot_label,
    ) = get_token_and_slot_label(context, response)
...
def create_examples(
    batch_dialogues_utterances_contexts_info,
    ...
    examples = []
    for dial_id, (contexts, responses) in enumerate(
        zip(
            batch_dialogues_utterances_contexts_info,
            batch_dialogues_utterances_responses_info,
        )
        ...
        for turn_id, (context, response) in enumerate(zip(contexts, responses)):
            ...
            action_label = (
                nlg_manager.get_action_id(response["act"])
                if (response is not None) and (response["act"] is not None)
                else 0
            )
            ...
            cur_ds = get_dialogue_state_sv_dict(
                context
            )  # Create DS here instead of in get_turn_label
            cur_ds = {
                **prev_ds,
                **cur_ds,
            }  # Merge with prev_ds, giving preference to cur_ds
            ...
            (
                ...
                context,  # context & response instd of turn
                response,
            ...
            )  # Not in original TripPy; The action idx the model is supposed to predict
...
def prepare_trippy_data(
    ...
    all_action_labels = torch.tensor(
        [e.action_label for e in examples], dtype=torch.long
    )
