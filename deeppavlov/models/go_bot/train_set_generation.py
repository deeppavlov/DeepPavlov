...
class TrainSetGeneration():
    ...
    def get_user_input(self) -> None:
        text = input('[INPUT] write a user sentence: ')
    ...
    def get_bot_output(self) -> None:
        ...
        for i, act in enumerate(self.templates.actions):
            print(i, act)
        id = self.get_id_input(prompt = 'type template number from the list: ',
                               valid_vals=list(range(len(self.templates.actions))))
...
if __name__ == '__main__':
    # if from console do import:
    #                           from deeppavlov.models.go_bot.go_bot_train_set_generation import TrainSetGeneration
    trainsetgen = TrainSetGeneration(template_path = 'dstc2-templates.txt',
                                     slot_path = 'dstc_slot_vals.json',
