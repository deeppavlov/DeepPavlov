...
class PtrNet:
    ...
    def __call__(self, init, match, hidden, mask):
        with tf.variable_scope(self.scope):
            ...
            _, logits2 = attention(match_do, state * dropout_mask, hidden, mask)
            return logits1, logits2


def dot_attention(inputs, memory, mask, hidden, keep_prob=1.0, scope="dot_attention"):
    with tf.variable_scope(scope):
        ...
        with tf.variable_scope("attention"):
            inputs_att = tf.layers.dense(d_inputs, hidden, use_bias=False, activation=tf.nn.relu)
            memory_att = tf.layers.dense(d_memory, hidden, use_bias=False, activation=tf.nn.relu)
            outputs = tf.matmul(inputs_att, tf.transpose(memory_att, [0, 2, 1])) / (hidden ** 0.5)
...
def simple_attention(memory, hidden, mask, keep_prob=1.0, scope="simple_attention"):
    ...
    with tf.variable_scope(scope):
        ...
        logits = tf.layers.dense(tf.layers.dense(memory_do, hidden, activation=tf.nn.sigmoid), 1, use_bias=False)
...
def attention(inputs, state, hidden, mask, scope="attention"):
    ...
    with tf.variable_scope(scope):
        ...
        logits = tf.layers.dense(tf.layers.dense(u, hidden, activation=tf.nn.sigmoid), 1, use_bias=False)
        logits = softmax_mask(tf.squeeze(logits, [2]), mask)
        att_weights = tf.expand_dims(tf.nn.softmax(logits), axis=2)
