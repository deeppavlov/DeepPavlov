...
from deeppavlov.skills.go_bot.templates import Templates, SQLTemplate
...
@register("go_bot_sql")
class GoalOrientedBotSQL(NNModel):
    def __init__(self, template_path,
        ...
        req_sub_dict_path = expand_path(req_sub_dict_path)
        with open(req_sub_dict_path, 'r') as dict_f:
            req_sub_dict = json.load(dict_f)
        log.info("[loading templates from {}]".format(template_path))
        self.templates = Templates(template_type, database, req_sub_dict).load(template_path)
    ...
    def _infer(self, context, db_result=None, prob=False):
        ...
        reply = self._decode_response(pred_id)
        if self.debug:
            log.debug(f"Reply is: {reply}")
        return reply
