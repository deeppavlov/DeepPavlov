...
class DefaultStatelessSkill(Skill):
    ...
    def __call__(self, utterances_batch: [list, tuple], history_batch: [list, tuple],
        ...
        if len(self.model.in_x) > 1:
            model_args = list(self.model.in_x)
            ...
            for utt_i, utterance in enumerate(utterances_batch):
                if not states_batch[utt_i]:
                    states_batch[utt_i] = {'expected_args': model_args, 'received_values': []}
                ...
                if states_batch[utt_i]['expected_args']:
                    response = EXPECTING_ARG_MESSAGE.format(states_batch[utt_i]['expected_args'].pop(0))
                    response_batch[utt_i] = response
                else:
                    infer_indexes.append(utt_i)
                    states_batch[utt_i] = None
                """
                if not states_batch[utt_i]:
                    states_batch[utt_i] = {
                        'expected_args': model_args[1:],
                        'received_values': [utterance]
                    }
                    response = EXPECTING_ARG_MESSAGE.format(model_args[1])
                    response_batch[utt_i] = response                    
                else:
                    states_batch[utt_i]['received_values'].append(utterance)
...
if __name__ == '__main__':
    ...
    result = skill(squad_infer_1, [], [])
    pprint(result)

    result = skill(squad_infer_2, [], result[3])
    pprint(result)