...
def get_server_params(model_config):
    server_params = copy(common_defaults)
    if check_nested_dict_keys(model_config, ['metadata', 'server_utils']):
        model_params = model_config['metadata']['server_utils']
        for param_name in server_params.keys():
            if model_params[param_name]:
                server_params[param_name] = model_params[param_name]
...
def start_model_server(model_config_path):
    model, model_config = init_model(model_config_path)
