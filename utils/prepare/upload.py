...
def upload(config_in_file: str, tar_name: str, tar_output_dir: str):

    if not Path.exists(Path(tar_output_dir)):
        raise RuntimeError(f'A folder {tar_output_dir} does not exist')

    if not Path.exists(Path(config_in_file)):
    ...
    full_tar_name = tar_output_dir + "/" + tar_name + '.tar.gz'
    if Path.exists(Path(full_tar_name)):
    ...
    print("Start tarring...")
    archive = tarfile.open(full_tar_name, "w|gz")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tar_name", help="name of the tar archive (without tar.gz extension)", default=None, required=False, type=str)
    parser.add_argument("--tar_output_dir", help="dir to save a tar archive", default="./", required=False, type=str)
