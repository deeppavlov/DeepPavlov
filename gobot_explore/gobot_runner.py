...
def main(config_fn, download):
    ...
    gobot_config = read_json(config_fn)
    ...
    gobot_config['train']['val_every_n_batches'] = 20  # evaluate on full 'valid' split each n batches
    gobot_config['train']['log_every_n_batches'] = 20  # evaluate on 20 batches of 'train' split every n batches

    json.dump(gobot_config, open('my_bot/gobot_config.json', 'wt'))

    print(f'BEFORE running {config_fn} {download}')
    train_model(gobot_config, download=download);
    print(f'AFTER running {config_fn} {download}')
